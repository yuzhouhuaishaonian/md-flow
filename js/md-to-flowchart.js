(function (global) {
  ...
  function extractMermaidDiagram(md) {
    ...
    const startRe = new RegExp("\\b(" + MERMAID_DIAGRAM_TYPES + ")\\b", "i");
  ...
  function sqlCreateTableToMermaid(sql) {
    ...
    const messageParts = [
      `已从 SQL 生成表结构图：${schema.tableName}`,
      `${schema.columnOrder.length} 个字段`,
    ];
    if (schema.tableComment) messageParts.push(schema.tableComment);
    if (schema.uniqueKeys.length) messageParts.push(`${schema.uniqueKeys.length} 个唯一键`);
    if (schema.indexes.length) messageParts.push(`${schema.indexes.length} 个索引`);
